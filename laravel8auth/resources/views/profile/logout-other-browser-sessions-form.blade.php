<x-jet-action-section>
    <x-slot name="title">
        {{ __('Brauseri sessioonid') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Halda ja logi välja enda hetkelistest sessioonidest') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Kui on vajalik saate välja logida kõikidest enda seadmetest. Kui Teil on tunne, et Teie konto võib olla kompromiseeritud, soovitame Teil vahetada enda parooli ära.') }}
        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Logi välja teistest brauseritest') }}
            </x-jet-button>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Sisestage palun enda parool, et kinnitada kõikidest teistest brauseritest väljalogimist.') }}
